#!/bin/bash
# Test script for break and continue validation
...
run_test() {
    ...
    # Generate actual output with our interpreter
    ...
    if diff -q "$WORKSPACE/${test_name}_expected.out" "$WORKSPACE/${test_name}_actual.out" > /dev/null; then
        ...
        echo "  ❌ FAIL"
        echo "  Expected:"
        cat "$WORKSPACE/${test_name}_expected.out" | head -10
        echo "  Actual:"
        cat "$WORKSPACE/${test_name}_actual.out" | head -10
        echo "  Diff:"
        diff "$WORKSPACE/${test_name}_expected.out" "$WORKSPACE/${test_name}_actual.out" | head -20
        FAIL=$((FAIL + 1))
    fi
    echo ""
}
